...
import { RefreshCw, CheckCircle2, AlertCircle, Loader2, Clock, DollarSign, Package } from 'lucide-react';
import { enqueueSyncJob, getSyncStatus } from '@/lib/api';

/**
 * Shopify Sync Button Component
 *
 * WHAT: Button that triggers Shopify data sync via async job queue
 ...
 * - Shopify-specific: shows revenue/profit stats
 *
 * REFERENCES:
 * - backend/app/services/shopify_sync_service.py
 * - Similar pattern: MetaSyncButton.jsx
 */
export default function ShopifySyncButton({ workspaceId, connectionId, onSyncComplete = null }) {
  ...
  const handleSync = async () => {
    ...
    try {
      ...
      setMessage(`Job queued • Syncing products, customers, orders...`);
      ...
      // Poll for completion
      let attempts = 0;
      const maxAttempts = 90; // 3 minutes max (Shopify syncs can take longer)
      ...
      const pollStatus = async () => {
        try {
          ...
          // Completed successfully
            ...
            // Calculate changes
            const newChanges =
              currentStatus.total_syncs_with_changes - (initialStatsRef.current?.changes || 0);
    ...
    <div className="space-y-2">
      ...
      {stats && status === 'success' && (
        <div className="text-xs text-neutral-500 space-y-1 mt-2 p-2 bg-neutral-50 rounded border border-neutral-200">
          ...
          <div>Total attempts: {stats.attempts} • Changes detected: {stats.changes}</div>
          {stats.hadChanges && (
            <div className="text-emerald-600 font-medium flex items-center gap-1">
              <Package className="w-3 h-3" />
              New orders/products synced!
            </div>
          )}
        </div>
      )}
