...
def get_pnl_statement(
    ...
    granularity: str = Query(
        "month", description="Granularity: month (now) or day (future)"
    ),
    ...
    manual_costs = (
        db.query(models.ManualCost)
        .filter(models.ManualCost.workspace_id == workspace_id)
        .all()
    )
    ...
    if compare:
        ...
        # Also filters to campaign-level only to avoid hierarchy double-counting.
        ...
        prev_latest_snapshots = (
            db.query(
                MS.entity_id,
                MS.metrics_date,
                func.max(MS.captured_at).label("max_captured_at"),
            )
            .join(E, E.id == MS.entity_id)
            .filter(E.workspace_id == workspace_id)
            .filter(E.level == "campaign")  # Campaign-level only
            .filter(MS.metrics_date.isnot(None))  # Exclude NULL dates
            .filter(MS.metrics_date >= prev_start)
            .filter(MS.metrics_date < prev_end)
            .group_by(MS.entity_id, MS.metrics_date)
            .subquery()
        )

        # Aggregate previous ad spend - only from latest snapshots
        prev_ad_query = (
            db.query(
            ...
            .join(
                prev_latest_snapshots,
                and_(
                    MS.entity_id == prev_latest_snapshots.c.entity_id,
                    MS.metrics_date == prev_latest_snapshots.c.metrics_date,
                    MS.captured_at == prev_latest_snapshots.c.max_captured_at,
                ),
            )
            .filter(E.workspace_id == workspace_id)
        ).one()
        ...
        prev_manual_by_category = get_allocated_costs(
            manual_costs, prev_start, prev_end
        )
        ...
        summary.compare = schemas.PnLComparison(
            revenue_delta_pct=(total_revenue - prev_total_revenue) / prev_total_revenue
            if prev_total_revenue > 0
            else None,
            spend_delta_pct=(total_spend - prev_total_spend) / prev_total_spend
            if prev_total_spend > 0
            else None,
            profit_delta_pct=(gross_profit - prev_profit) / abs(prev_profit)
            if prev_profit != 0
            else None,
    ...
    latest_snapshots = (
        ...
        .filter(
            E.level == "campaign"
        )  # Campaign-level only to avoid hierarchy double-counting
        .filter(MS.metrics_date.isnot(None))  # Exclude NULL dates
        .filter(MS.metrics_date >= period_start)
        .filter(MS.metrics_date < period_end)
        .group_by(MS.entity_id, MS.metrics_date)
        .subquery()
    )
    ...
    daily_data = (
        db.query(
            MS.metrics_date.label("date"),
            func.sum(MS.revenue).label("revenue"),
            func.sum(MS.spend).label("spend"),
        )
        .join(E, MS.entity_id == E.id)
        .join(
            latest_snapshots,
            and_(
                MS.entity_id == latest_snapshots.c.entity_id,
                MS.metrics_date == latest_snapshots.c.metrics_date,
                MS.captured_at == latest_snapshots.c.max_captured_at,
            ),
        )
        .filter(E.workspace_id == workspace_id)
        .group_by(MS.metrics_date)
        .order_by(MS.metrics_date)
        .all()
    )
    ...
    return schemas.PnLStatementResponse(
        summary=summary, rows=rows, composition=composition, timeseries=timeseries
    )
...
def create_manual_cost(
    ...
    allocation_date = (
        cost.allocation.date if cost.allocation.type == "one_off" else None
    )
    allocation_start = (
        cost.allocation.start_date if cost.allocation.type == "range" else None
    )
    allocation_end = (
        cost.allocation.end_date if cost.allocation.type == "range" else None
    )
...
def list_manual_costs(
    ...
    costs = (
        db.query(models.ManualCost)
        .filter(models.ManualCost.workspace_id == workspace_id)
        .order_by(models.ManualCost.created_at.desc())
        .all()
    )
...
def update_manual_cost(
    ...
    if updates.allocation is not None:
        ...
        db_cost.allocation_date = (
            updates.allocation.date if updates.allocation.type == "one_off" else None
        )
        db_cost.allocation_start = (
            updates.allocation.start_date
            if updates.allocation.type == "range"
            else None
        )
        db_cost.allocation_end = (
            updates.allocation.end_date if updates.allocation.type == "range" else None
        )
