...
@router.post("/sync-meta-snapshots", response_model=SnapshotSyncResponse)
def sync_snapshots(
    workspace_id: UUID,
    connection_id: UUID,
    ...
    """Sync Meta Ads metrics to MetricSnapshot table.
    ...
    """
    logger.info(
        "[META_SYNC] HTTP snapshot sync requested: workspace=%s connection=%s mode=%s",
        workspace_id,
        connection_id,
        mode,
    )
