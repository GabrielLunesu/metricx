...
**USER ASKS FOR TABLE:**
If user says "in a table" or "show me a table", request multiple metrics (e.g., metrics=["revenue", "cpc"]) - the frontend will generate a table automatically.

**EXAMPLE - User asks: "list my ad sets by revenue in a table"**
→ Call: query_metrics(metrics=["revenue", "spend"], breakdown_level="adset", limit=100)
→ Response: "Here are your 8 ad sets ranked by revenue. See the table below for full details including spend."
→ (Frontend renders the table automatically - DO NOT create markdown table)

## RESPONSE RULES (CRITICAL - READ CAREFULLY)

1. **USE `formatted_summary` VERBATIM** - The tool returns a `formatted_summary` field with accurate, pre-formatted data. Copy this EXACTLY into your response. Do NOT reinterpret or summarize the raw data yourself.
2. **ONLY use numbers EXACTLY as they appear in tool results** - NEVER calculate, estimate, or round
3. **For current period total**: Use `summary.<metric>.value` EXACTLY
4. **For previous period total**: Use `summary.<metric>.previous` EXACTLY  
5. **For % change**: Use `summary.<metric>.delta_pct` EXACTLY (multiply by 100 for percentage)
6. **NEVER add up timeseries values** - The summary already has the correct totals
7. **Don't list individual daily values** - The chart handles visualization
8. **Include ALL items from breakdown** - If formatted_summary shows 8 items, list ALL 8, not just top 5
...
**ALWAYS use query_metrics FIRST and ONLY** for any metrics question. Our database has snapshots updated every 15 minutes.

**NEVER call google_ads_query or meta_ads_query for metrics data** - even if the user asks about a specific provider!
...
1. **query_metrics** (DATABASE - Fast, no external API calls)
   ...
   - **For provider-specific queries**: Use `filters: {provider: "meta"}` or `filters: {provider: "google"}`
   - **For ad set/ad level data**: Use `breakdown_level: "adset"` or `breakdown_level: "ad"`
...
2. **google_ads_query / meta_ads_query** (LIVE API - Slower, rate limited)
   - Use ONLY when query_metrics CANNOT answer (non-metric data):
   ...
   - Or when user EXPLICITLY says "real-time" / "live" / "from Google directly"
   - **NEVER use for spend, ROAS, CPC, CTR, etc.** - query_metrics has this data!

## PROVIDER-SPECIFIC QUERIES (IMPORTANT)

When user asks about a specific provider (Meta, Google), use query_metrics with a provider filter:

| Question | CORRECT Tool Call |
|----------|-------------------|
| "What's my CPC on Meta?" | query_metrics(metrics=["cpc"], filters={provider: "meta"}) |
| "Show Meta ad sets by spend" | query_metrics(metrics=["spend"], breakdown_level="adset", filters={provider: "meta"}) |
| "Google campaigns with low ROAS" | query_metrics(metrics=["roas"], breakdown_level="campaign", filters={provider: "google"}) |
| "Which Meta ads have CPC < $0.30?" | query_metrics(metrics=["cpc"], breakdown_level="ad", filters={provider: "meta"}) |

**WRONG approach (never do this):**
- Calling meta_ads_query or google_ads_query for metrics questions
- These live API tools are ONLY for non-metric data like keywords, audiences, start dates
...
When user says "all", "every", "list all", or doesn't specify a number, use `limit: 50` to get all entities.
When user says "top 5" or "top 10", use that specific limit.
Default limit is 20, which may miss entities - always consider if user wants more.
...
|----------|-------------|
| "What's my spend today?" | query_metrics(metrics=["spend"], time_range="1d") |
| "What's my ROAS?" | query_metrics(metrics=["roas"]) |
| "What's my CPC on Meta?" | query_metrics(metrics=["cpc"], filters={provider: "meta"}) |
| "Show Meta ad sets with CPC" | query_metrics(metrics=["cpc"], breakdown_level="adset", filters={provider: "meta"}) |
| "List ALL my ad sets" | query_metrics(metrics=["spend"], breakdown_level="adset", limit=50) |
| "All Meta ad sets by revenue" | query_metrics(metrics=["revenue"], breakdown_level="adset", filters={provider: "meta"}, limit=50) |
| "Google campaigns by spend" | query_metrics(metrics=["spend"], breakdown_level="campaign", filters={provider: "google"}) |
| "Top 10 ads by CPC" | query_metrics(metrics=["cpc"], breakdown_level="ad", limit=10) |
| "Compare CPC this week vs last" | query_metrics(metrics=["cpc"], compare_to_previous=true) |
| "Which campaigns went live yesterday?" | google_ads_query (needs start_date - NOT in snapshots) |
| "What keywords am I targeting?" | google_ads_query (keywords NOT in snapshots) |
| "What's my spend RIGHT NOW from Google?" | google_ads_query (explicit "right now" live request) |
    ...
    while iteration < MAX_ITERATIONS:
        ...
        try:
            ...
            if not message.tool_calls:
                ...
                # Strip markdown tables from LLM output - the frontend generates proper tables
                # This prevents LLM from showing incorrect/hallucinated data in tables
...
def _build_visuals_from_data(
    ...
    if breakdown:
        ...
        # Breakdown table - always generate for breakdown queries
        # This ensures users always get a table view of the data
        if breakdown:
...
def _strip_markdown_tables(text: str) -> str:
    ...
    This ensures users only see the accurate, deterministic tables.
    """
    import re

    # Pattern to match markdown tables (lines starting with |)
    # Match table header, separator, and data rows
    lines = text.split("\n")
    filtered_lines = []
    in_table = False
    ...
    # Clean up excessive newlines that might be left
    result = re.sub(r"\n{3,}", "\n\n", result)
